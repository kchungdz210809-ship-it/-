...
local AtlasUi = Atlas.new({
    Name = "🌴 Island Tribes Script made by Casey x Elite 🌴";
    ConfigFolder = "IslandTribes"; 
    Credit = "Credits: zvvpe & Casey x Elite";
    Color = Color3.fromRGB(66, 188, 245);
    UseLoader = false;
    Bind = "LeftBracket";
    FullName = "Island Tribes monkey";
        CheckKey = function(Elitekey)
        return Elitekey == "MungiScripting"
    end;
    Discord = "Discord user : vazqiii";
})
...
MainSection:CreateButton({
    ...
    Callback = function()
        ...
        if IsPlayerAlive(LocalPlayer) then
 			local oldpos = workspace.Replicators.NonPassive.ykwhatustinky.HumanoidRootPart.CFrame
			local myroot = workspace.Replicators.NonPassive.ykwhatustinky.HumanoidRootPart
		for i=1, 100 do
...
AnythingInChestSection:CreateButton({
    ...
    Callback = function()
        ...
        for i=1, getgenv().AmountOfChestInserts do
            RemoteEvents['UpdateStorageChest']:FireServer(GetFilteredChest, false, SWITCHEDITEMSTABLE[getgenv().ItemToPutInChest])
        end
    end
})
...
local ChestSpawningSection = Chests:CreateSection('Spawning stuff (u will need a person near u to spawn) ')


ChestSpawningSection:CreateButton({
    Name = 'Spawn pirate ship 3 wood to spawn',
        Callback = function()
           if IsPlayerAlive(LocalPlayer) then
        local function GetClosestTrapPlayer()
            local range = 70
...
local ChestOpeningSection = Chests:CreateSection('Sleighs Spawn')

ChestOpeningSection:CreateButton({
    Name = 'spawn sleigh (REQ. | 3 Gold, 5 Diamonds, 150🍬)',
        Callback = function()
           if IsPlayerAlive(LocalPlayer) then
        local function GetClosestTrapPlayer()
            local range = 70
...
ChestOpeningSection:CreateButton({
 Name = 'Spawn Sleigh v2 (REQ. | 5 Ruby, 100🍬)',
    Callback = function(Value)
        if IsPlayerAlive(LocalPlayer) then
        local function GetClosestTrapPlayer()
            local range = 70
...
ChestOpeningSection:CreateButton({
 Name = 'Spawn Grinchs Sleigh (REQ. | 6 Coal, 5 Zen, 200🍬) ',
    Callback = function(Value)
        if IsPlayerAlive(LocalPlayer) then
        local function GetClosestTrapPlayer()
            local range = 70
...
local ChestOpeningSection = Chests:CreateSection('Crafting')

ChestOpeningSection:CreateButton({
    Name = 'Craft Pump Shield',
    Callback = function()
        RemoteEvents['CraftItem']:FireServer(379)
    end
})
...
CreditSection:CreateParagraph("vazqiii - discord")
CreditSection:CreateParagraph("EliteChaosss - Roblox")
CreditSection:CreateParagraph("casemm25590alt - Roblox")
